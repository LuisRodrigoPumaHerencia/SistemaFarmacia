...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        header("location:../account_settings.php?er=El correo $shopemail ya esta en uso por favor seleccione otro correo electrónico");
    }
} else {
    include '../../config/db_config.php';
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        header("location:../account_settings.php?er=El número de teléfono $Shopphone ya está en uso");
    }
} else {
    include '../../config/db_config.php';
